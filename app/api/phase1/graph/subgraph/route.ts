...
export async function GET(request: Request) {
  ...
  const path =
    ...
    `/graph/nodes/${selectedCandidateId}/subgraph?max_depth=2&limit=40`;
  return forwardBackendJson({
    path,
    parse: parseStudentSubgraphResponse,
    scope,
    assertScope: assertResponseScope,
    assertValue: (value) => {
      if (value.root_candidate_id !== selectedCandidateId) {
        throw new Error("subgraph root does not match request");
      }
    },
  });
}
