...
def ledger(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, *, requests: int = 2, seconds: float = 30
) -> RecordingBudget:
    run = str(uuid4())
    path = tmp_path / "ledger.sqlite"
    budget = RecordingBudget.provision(
        path, run, requests=requests, seconds=seconds, output_tokens=100, request_bytes=65536
    )
...
def test_structured_scope_original_transfer_and_topic_switch() -> None:
    ...
    for width in (1e-10, 1.5e-10, 1e-10 * 1.5, 2e-10, 0.137e-9):
        assert BarrierTask.model_validate({**original, "barrier_width_m": width})
    for change in (
        {"energy_eV": 12},
        {"barrier_width_m": -2e-10},
        {"particle_mass_kg": 1e-30},
        {"potential": "semi-infinite"},
    ):
        with pytest.raises(ValueError):
...
def test_corrupt_missing_and_wrong_run_ledgers_fail_closed(tmp_path: Path) -> None:
    ...
    budget = RecordingBudget.provision(
        path, "one", requests=1, seconds=10, output_tokens=100, request_bytes=100
    )
...
async def test_same_retriever_switches_topics_without_bypassing_barrier_gate(
    ...
    review = BarrierSourceReview(
        **{
        ...
        review_reference="TEST DOUBLE ONLY",
    ...
    path.write_text(
        ReviewManifest(schema_version=1, test_double=True, reviews=(review,)).model_dump_json()
    )
    loaded = load_reviews(
        path, hashlib.sha256(path.read_bytes()).hexdigest(), allow_test_double=True
    )
