...
import { assertDiagnosisScene, assertFullJourney } from '../scripts/video-parity/acceptance.mjs';

test('an observed fallback cannot stand in for a model diagnosis in full acceptance', () => {
  assert.throws(() => assertDiagnosisScene({diagnosis: {status: 'observed'}}), /unavailable/);
  assert.doesNotThrow(() => assertDiagnosisScene({diagnosis: {
    status: 'model_inference', observation_basis: ['student_attempt'],
    first_error: {kind: 'physical_interpretation_error', description: 'The test answer infers zero wavefunction from E<V0.'},
  }}));
});
