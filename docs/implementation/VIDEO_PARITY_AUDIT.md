...
追加复核：新任务首屏空计算区使发送按钮落屏，已限制其在真实轮次结果之后展示，并新增两尺寸真实页面断言。全新主线复跑遇到外部代码生成超时，保留 INCONCLUSIVE；详见验收报告顶部，不能声明本次复跑通过。

最新 Flash 复核完成两条全新真实主线。新增服务端统一文本路由及非思考配置；修复 Coding Agent 未携带旧程序的修复请求，以及 Teach-Back 把历史错误继续算入当前评价的问题。当前观点独立评价不能借旧成功覆盖新错误；有效部分回答再结合历史评价。错误→修正及五类输入均经真实 Flash 调用验证。详细事件、失败过程和执行边界见验收报告最新段落。
