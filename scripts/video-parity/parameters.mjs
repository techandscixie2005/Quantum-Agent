...
try{
 ...
 await expect(page.getByTestId('coding-artifact')).toBeVisible();if(last.code_artifact?.verification.status!=='pass')throw new Error('Coding verification did not pass');
  ...
  // A restored derivation turn may have no current numeric artifact. Establish
  // a real baseline before asserting that changed parameters invalidate it.
  await turn(()=>page.getByRole('button',{name:'确定性参考重算',exact:true}).click(),'parameter-baseline');
  if(!last.scientific_results.some(r=>r.kind==='rectangular_barrier_tunnelling' && r.status==='pass')) throw new Error('No verified baseline for parameter invalidation');
 }
 const computations=[];
