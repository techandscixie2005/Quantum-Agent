...
import { assertFullJourney } from './acceptance.mjs';
...
let index=0,last=null;const timings=[];const turns=[];
...
try{
 ...
 if(last.learning_native.phase==='reconstruction_required') await teach('两端分别匹配 ψ 和 ψ′，给出四个线性方程，确定 B/A、C/A、D/A、F/A；概率流 j=(ℏ/m)Im(ψ*ψ′)，因此相同外势与质量下 T=|F/A|²。κ由能量差和质量决定，宽度改变只改变κa。','08-clarification');
 ...
 const lock=await page.evaluate(async ({episode,course,edition})=>{
  ...
  return {state,historyStatus:history.status,sourceStatus:source.status};
