...
try {
 for(const [width,height] of [[1920,1080],[1366,768]]){
  ...
  for(const [stage,label] of [['sources','课程依据'],['bridge','推导补桥'],['verify','科学计算'],['teach_back','Teach-Back'],['transfer','Transfer'],['solo','Solo']]){
   ...
   if(stage==='sources'){
    const opener=review.getByRole('button',{name:'打开课程来源'}).first();await opener.click();
    ...
    reports.push({width,stage:'original-source',...fetched});await page.screenshot({path:`${out}/${width}-source-modal.png`});
